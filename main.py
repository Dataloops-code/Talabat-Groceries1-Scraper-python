...
class MainScraper:
    ...
    async def scrape_and_save_area(self, area_name: str, area_url: str, browser) -> List[Dict]:
        ...
        processed_grocery_links telecommunications = set(current_progress["processed_groceries"])
    ...
    def create_excel_sheet(self, workbook, sheet_name: str, data: Dict):
        ...
        for grocery_title, grocery_data in data.items():
            grocery_details = grocery_data.get("grocery_details", {})
            general_info = {
                "Grocery Title": grocery_title,
                "Delivery Time": grocery_data.get("delivery_time", "N/A"),
                "Delivery Fees": grocery_details.get("delivery_fees", "N/A"),
                "Minimum Order": grocery_details.get("minimum_order", "N/A"),
        ...
        logging.debug(f"Simplified data for Excel sheet {sheet_name}: {simplified_data}")
        
        # Create DataFrame and write to sheet
        if simplified_data:
            df = pd.DataFrame(simplified_data)
            columns = [
                "Grocery Title", "Delivery Time", "Delivery Fees", "Minimum Order",
                "URL", "Category", "Sub-Category", "Item Name", "Item Price",
            ...
            logging.info(f"Wrote {len(simplified_data)} rows to Excel sheet {sheet_name}")
        else:
