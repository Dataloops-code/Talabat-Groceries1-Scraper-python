...
class MainScraper:
    ...
    async def scrape_and_save_area(self, area_name: str, area_url: str, browser):
        ...
        print(f"Starting scrape for {area_name}")
        ...
        try:
            ...
            await page.goto(area_url, timeout=240000, wait_until="domcontentloaded")
            await page.wait_for_selector('//div[@class="jsx-4082160271 category-list"]', timeout=60000)

            categories = await page.query_selector_all('//div[@class="jsx-4082160271 category-list"]//a[@class="jsx-4082160271 category-item"]')
            print(f"Found {len(categories)} categories")

            talabat_scraper = TalabatScraper(self)

            for i, category in enumerate(categories):
                try:
                    category_name = await category.query_selector('h3')
                    category_name = await category_name.inner_text() if category_name else f"Category {i+1}"
                    print(f"Processing category: {category_name}")
                    category_link = self.base_url + await category.get_attribute('href')
                    print(f"Category link: {category_link}")

                    category_data = await talabat_scraper.scrape_category(category_link, context)
                    category_data["category_name"] = category_name
                    category_data["category_link"] = category_link

                    output_file = os.path.join(self.output_dir, f"{area_name}_{category_name.replace(' ', '_')}.json")
                    with open(output_file, 'w', encoding='utf-8') as f:
                        json.dump(category_data, f, ensure_ascii=False, indent=2)
                    print(f"Saved category data to {output_file}")

                    self.update_progress(category_name, category_link, "completed")
                except Exception as e:
                    print(f"Error processing category {category_name}: {e}")
                    logging.error(f"Error processing category {category_name}: {e}")
                    self.update_progress(category_name, category_link, "failed")
    ...
    def update_progress(self, category_name, category_link, status):
        progress_data = {
        ...
        try:
            with open(self.CURRENT_PROGRESS_FILE, 'a', encoding='utf-8') as f:
                json.dump(progress_data, f, ensure_ascii=False)
                f.write('\n')
            if status == "completed":
                with open(self.SCRAPED_PROGRESS_FILE, 'a', encoding='utf-8') as f:
                    json.dump(progress_data, f, ensure_ascii=False)
                    f.write('\n')
        except Exception as e:
...
class TalabatScraper:
    def __init__(self, main_scraper):
        ...
        self.base_url = main_scraper.base_url
        self.browser = main_scraper.browser

    async def scrape_category(self, category_link: str, context):
        sub_categories = await self.extract_sub_categories(category_link, context)
        category_data = {
            "sub_categories": []
        }
        ...
        for sub_category in sub_categories:
            ...
            sub_category_link = sub_category["sub_category_link"]
            print(f"  Processing sub-category: {sub_category_name}")

            items = await self.extract_all_items_from_sub_category(sub_category_link, context)
    ...
    async def extract_sub_categories(self, category_link: str, context):
        print(f"Extracting sub-categories from: {category_link}")
        async with self.main_scraper.semaphore:
            retries = 3
            while retries > 0:
                try:
                    page = await context.new_page()
                    await page.goto(category_link, timeout=240000, wait_until="domcontentloaded")
                    await page.wait_for_selector('//div[@class="jsx-4082160271 category-list"]', timeout=60000)

                    sub_categories = []
                    sub_category_elements = await page.query_selector_all('//div[@class="jsx-4082160271 category-list"]//a[@class="jsx-4082160271 category-item"]')
                    print(f"    Found {len(sub_category_elements)} sub-categories")

                    for i, element in enumerate(sub_category_elements):
                        try:
                            sub_category_name = await element.query_selector('h3')
                            sub_category_name = await sub_category_name.inner_text() if sub_category_name else f"Sub-Category {i+1}"
                            sub_category_link = self.base_url + await element.get_attribute('href')
                            print(f"    Sub-category: {sub_category_name}, Link: {sub_category_link}")
                            sub_categories.append({
                                "sub_category_name": sub_category_name,
                                "sub_category_link": sub_category_link
                            })
                        except Exception as e:
                            print(f"    Error processing sub-category {i+1}: {e}")
                            logging.error(f"Error processing sub-category {i+1}: {e}")

                    await page.close()
                    return sub_categories
                except Exception as e:
                    print(f"Error extracting sub-categories from {category_link}: {e}")
                    retries -= 1
                    print(f"Retries left: {retries}")
                    if 'page' in locals():
                        await page.close()
                    await asyncio.sleep(10 * (2 ** (3 - retries)))  # Exponential backoff
            print(f"Failed to extract sub-categories from {category_link} after all retries")
            return []

    async def extract_all_items_from_sub_category(self, sub_category_link: str, context):
                    ...
                    for page_number in range(1, total_pages + 1):
                        ...
                        for i, element in enumerate(item_elements):
                            try:
                                ...
                                item_details = await self.extract_item_details_with_context(item_page, item_link)
                                ...
                                await asyncio.sleep(3)  # Increased delay to prevent overload
                            except Exception as e:
    ...
    async def extract_item_details_with_context(self, page, item_link):
        print(f"Attempting to extract item details for link: {item_link}")
        self.main_scraper.active_contexts += 1
        logging.info(f"Opening new page, active contexts: {self.main_scraper.active_contexts}")
        async with self.main_scraper.semaphore:
            retries = 3
            while retries > 0:
                try:
                    ...
                    # Old price
                    item_old_price = None
                    old_price_elements = await page.query_selector_all(
                        '//span[@class="jsx-2582391157 old-price"] | '
                        '//span[contains(@class, "old-price")] | '
                        '//div[contains(@class, "original-price")]'
                    )
                    if old_price_elements:
                        item_old_price = await old_price_elements[0].inner_text()
                    print(f"Item old price: {item_old_price}")

                    # Offer
                    item_offer = None
                    offer_elements = await page.query_selector_all(
                        '//div[@class="jsx-2582391157 offer"] | '
                        '//span[contains(@class, "offer")] | '
                        '//div[contains(@class, "discount")]'
                    )
                    if offer_elements:
                        item_offer = await offer_elements[0].inner_text()
                    ...
                    # Delivery time
                    delivery_time = "N/A"
                    delivery_elements = await page.query_selector_all(
                        '//span[contains(@class, "delivery-time")] | '
                        '//div[contains(@class, "delivery-info")]'
                    )
                    if delivery_elements:
                        delivery_time = await delivery_elements[0].inner_text()
                    ...
                    print(f"Timeout error for {item_link}: {e}")
                    retries -= 1
                    print(f"Retries left: {retries}")
                    try:
                    ...
                    print(f"Unexpected error for {item_link}: {e}")
                    retries -= 1
                    print(f"Retries left: {retries}")
                    if retries > 0:
                        await asyncio.sleep(10 * (2 ** (3 - retries)))
                    ...
                    self.main_scraper.active_contexts -= 1
                    logging.info(f"Closed page, active contexts: {self.main_scraper.active_contexts}")
...
async def main(area_name: str, area_url: str):
    async with async_playwright() as p:
        browser = await p.chromium.launch(headless=True, args=['--disable-gpu', '--no-sandbox', '--disable-dev-shm-usage'])
        scraper = MainScraper(area_name)
        await scraper.scrape_and_save_area(area_name, area_url, browser)
        await browser.close()

if __name__ == "__main__":
    import argparse

    parser = argparse.ArgumentParser(description="Scrape Talabat grocery data.")
    ...
    asyncio.run(main(args.area_name, args.url))
...
#                 # Delay before navigation to avoid rate-limiting
...
#                     '//div[contains(@class, "product-details")]'
#                 )
#                 await page.wait_for_selector(critical_selector, timeout=60000)
        
#                 # Scroll to ensure all elements are loaded
#                 await page.evaluate("window.scrollTo(0, document.body.scrollHeight)")
#                 await page.wait_for_timeout(2000)
        
#                 # JavaScript-based fallback for price
#                 item_price = await page.evaluate("""
#                     () => {
#                         const price = document.querySelector('[class*="price"], [data-testid*="price"]')?.innerText;
...
#                             break
#                 if item_description == "N/A":
#                     # JavaScript-based fallback for description
#                     item_description = await page.evaluate("""
#                         () => {
...
#                 if retries > 0:
#                     await asyncio.sleep(5)
#                     print("Retrying with reduced timeout...")
#                     critical_selector_timeout = 45000  # Reduced timeout for retries
#                 continue
